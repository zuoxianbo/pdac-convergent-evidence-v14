...
OUTDOC = os.environ.get("V14_DOCX", os.path.join(RES, "Pancreatic_cancer_convergent_evidence_NCS_V14.docx"))
...
P((f"Multimodal evidence integration is widely assumed to improve target prioritisation, yet \u201cmore evidence\u201d is not "
   f"synonymous with \u201cmore information\u201d. We formalise and test a task-dependent principle: fusion adds information only when "
   f"the target property is conjunctive and not already encoded by a single strong predictor axis. We rank "
   f"{B['meta']['n_genes']:,} genes with a transparent Evidence Convergence Score (ECS = D x (1+alpha x PHI)) that augments "
   f"STRING[[string]] network topology with five orthogonal evidence layers, and benchmark it against eight scorers across six endpoints[[depmap]]. "
   f"(1) On topology-encoded endpoints STRING alone is optimal and fusion is redundant (pan-dependency AUROC "
   f"{f(auc('STRING_centrality','E1_pan_dependency'))}, genetic-disease {f(auc('STRING_centrality','E4_genetic_disease'))}, "
   f"established clinical targets {f(e6_str)}). (2) On the conjunctive actionable-target endpoint ECS wins "
   f"(AUROC {f(auc('ECS_proposed','E3_actionable_target'))} vs {f(auc('STRING_centrality','E3_actionable_target'))}; "
   f"DeLong delta-AUROC = {f(dl['delta'])} [{f(dl['ci_lo'])}, {f(dl['ci_hi'])}], p < 0.001). Component attribution shows the "
   f"entire gain is attributable to a single layer \u2014 druggability (STRING+druggability reaches {f(m2)}, whereas adding "
   f"genetics or tissue adds nothing; full ECS {f(m6)} is in fact below the targeted STRING+druggability baseline). "
   f"(3) A formal negative-control experiment confirms the principle: established clinical targets are already network-central "
   f"(STRING {f(e6_str)} > ECS {f(e6_ecs)}), so convergence adds little beyond topology \u2014 exactly as predicted. "
   f"(4) ECS re-ranks genes so actionable targets rise from mean rank {f(B['ecs_specific']['e3pos_rank_string'],0)} "
   f"(STRING) to {f(B['ecs_specific']['e3pos_rank_ecs'],0)} (ECS); the ECS-specific set is not significantly more actionable "
   f"than a STRING-specific control (Fisher OR = {f(fish['odds_ratio'],2)}, p = {fish['p_value']:.2f}), so ECS's value is "
   f"recovery and re-ordering, not superior raw enrichment. We conclude: evidence is not information; the right \u2014 "
   f"conjunctive \u2014 evidence is."), space=8)

# ===================== INTRODUCTION =====================
H("1. Introduction", 1)
P((   "Target prioritisation[[behan]] for pancreatic ductal adenocarcinoma (PDAC)[[bailey]] is constrained by a small repertoire of "
   "actionable dependencies. A natural response is to fuse every available omics layer \u2014 network[[string]], genetics[[ot]], "
...
P(("\u201cEvidence is not information.\u201d The information a new layer contributes is bounded by how much predictive variation it "
   ...
   "weakly correlated biological attributes \u2014 only integration can recover it. We state this as a formal principle:"))
...
P(("We operationalise this with an Evidence Convergence Score (ECS). Each gene receives a topology-dependent base D (a weighted "
   "STRING[[string]] centrality) and a convergence term PHI averaging five support layers (cancer-driver mutation, PDAC genetic association, "
...
P((f"Across eight scorers and six endpoints (Fig. 2), the best scorer depends entirely on the endpoint's structure. STRING[[string]] "
   f"dominates the topology-encoded tasks: pan-dependency[[depmap]] (E1, AUROC {f(auc('STRING_centrality','E1_pan_dependency'))}), "
   f"genetic disease (E4, {f(auc('STRING_centrality','E4_genetic_disease'))}), zero-shot CRC transfer[[depmap_ccle]] (E5, "
   f"{f(auc('STRING_centrality','E5_crc_transfer'))}) and established clinical targets (E6, {f(e6_str)}). ECS wins only on the "
   f"two endpoints that are intrinsically conjunctive \u2014 selective dependency (E2, {f(auc('ECS_proposed','E2_selective_dependency'))} "
   ...
   "not a universal lift: it helps only where the target property is conjunctive and not already captured by topology, and the "
   "pattern of wins and losses is itself the result."))
...
P((f"A single \u201cECS minus druggability\u201d comparison is misleading, so we decompose the gain nestedly (Fig. 3). Starting from "
   ...
   f"First, the entire E3 gain is attributable to the druggability component \u2014 no other layer contributes. Second, ECS is not the "
...
P((f"Endpoint E6 is an external, prospectively defined negative-control: PDAC targets derived from ClinicalTrials.gov[[ctgov]] interventional "
   f"trials (n = {B['meta']['e6_n_genes']} genes; mapping detailed in Methods), never used inside ECS. Here STRING "
   f"({f(e6_str)}) outperforms ECS ({f(e6_ecs)}). The interpretation is not that ECS \u201cfails\u201d but that these targets are already "
   "highly network-central \u2014 they were selected by a century of trial-and-error that converged on hubs \u2014 so multimodal convergence "
   "adds little beyond topology. This is the expected, confirmatory behaviour of a task-dependent framework: when the endpoint is "
   "already encoded by the strong axis, fusion is redundant. E6 is thus a negative-control validation of the hypothesis, not an "
   "external success of ECS."))
...
P(("We set out to test a single idea: multimodal evidence integration is task-dependent[[pushpakom]]. The data support it cleanly. Integration "
   ...
   "whereas a method that improves only the conjunctive endpoint demonstrates a principled, generalisable boundary."))
...
P(("\u201cEvidence is not information; the right \u2014 conjunctive \u2014 evidence is.\u201d The task-dependent boundary we demonstrate is not "
   "specific to PDAC: any prioritisation problem can be audited by asking whether its endpoint is conjunctive[[cheng]] and whether the added "
...
# ===================== METHODS (brief) =====================
H("4. Methods", 1)
P(("Evidence layers. Eleven normalised layers (STRING[[string]] v11 network centrality; mutation frequency; IMPC[[impc]] animal knock-out; genetic "
   "constraint; cancer-driver[[tamborero]] mutation; Open Targets[[ot]] PDAC genetic association; DrugBank[[dgidb]] druggability; HPA[[hpa_path]] PDAC prognostic and pan-tissue[[hpa]] "
   "specificity expression) were drawn from the platform knowledge base and z-scored per layer. ECS = D x (1 + 0.6 x PHI) with "
   "D = weighted mean(STRING 0.80, mutation 0.10, IMPC 0.10) and PHI = mean of the five support layers."))
P(("Endpoints. E1 pan-dependency and E5 zero-shot CRC transfer from DepMap[[depmap]] 26Q1 essentiality[[depmap_meyers]][[depmap_dempster]] (PDAC vs CRC lineages mapped via the 2018 CCLE[[depmap_ccle]] "
   "CCLE annotation); E2 PDAC-selective dependency from four concordant definitions; E3 essential intersect druggable; E4 Open Targets "
   "PDAC top-500 (constructive redundancy test \u2014 OT genetics is an ECS layer); E6 independent clinical targets."))
P(("E6 mapping (transparency). ClinicalTrials.gov[[ctgov]] v2 PDAC interventional trials were retrieved; each intervention was mapped intervention "
   "\u2192 drug \u2192 molecular target \u2192 HGNC gene using a curated drug\u2013target dictionary (mapping date 2026-08; combination therapies "
   "contributed each component; agents with unknown mechanism or non-gene targets were excluded). The resulting gene set was intersected "
   "with the scored universe and never used in ECS construction, weights, hyperparameters or candidate selection."))
P(("Statistics. Metrics[[fawcett]]: AUROC, AUPRC, NDCG@100, precision@50/100, ECE, with 1,000\u20132,000-replicate bootstrap CI. The E3 delta used "
   "the DeLong[[delong]] (1988) paired test with the standard structural-component covariance, evaluated on the same gene set for both scorers; "
   "permutation[[north]] control shuffled support layers 1,000 times; Fisher's exact test compared ECS-specific vs STRING-specific actionable "
   "enrichment; component attribution used a nested STRING\u2192+support decomposition. No endpoint leaked into another (see 3)."))
